...
=item B<printi>([FILEHANDLE], FORMAT, PAIRS|HASH)
...
=item B<printp>([FILEHANDLE], FORMAT, PAIRS|HASH)
...
then fed into L<sprinti()|String::Print/"FUNCTIONS">.  Be careful with the length of the LIST:
...
=head2 Why C<printi()>, not C<printf()>?
...
C<printf()> uses positional values, where L<printi()|String::Print/"FUNCTIONS"> uses names
...
in Chinese.  The L<printi()|String::Print/"FUNCTIONS"> implementation will use Unicode::GCString
...
=head2 Interpolation, the serialization of variables
...
The standard L<printi()|String::Print/"FUNCTIONS"> is without smart behavior
...
can specify abstract formatting.  Using L<printp()|String::Print/"FUNCTIONS"> makes it a little
...
This module is part of String-Print distribution version 0.10,
built on April 15, 2013. Website: F<http://perl.overmeer.net/log-report/>
